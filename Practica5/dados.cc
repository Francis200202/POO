...
ostream &operator<<(ostream &stream, Dados &d){
	stream<<"Valor dado 1: "<<d.getDado1()<<endl;
	stream>>"Valor dado 2: "<<d.getDado2()<<endl;
	return stream;
}
...
istream &operator>>(istream &stream, Dados &d){
	int aux;
	do{
		cout<<"Introduce dado1: "<<endl;
		stream>>aux;
	}while(!d.setDado1(aux));

	do{
		cout<<"Introduce dado2: "<<endl;
		stream>>aux;
	}while(!d.setDado2(aux));
